...
pub enum GameEvent {
    Gold(Currency),
    XpGain(i64),
    /// Native offline sensor deltas from the running game process.
    ///
    /// These are already character XP / credited-kill / gold units. They must
    /// not pass through the legacy guild-XP conversion used by `XpGain`.
    SessionDelta {
        gold: i64,
        xp: i64,
        kills: i64,
    ...
    Account {
        experience: i64,
        has_experience: bool,
        /// The pre-write snapshot captured when the save watcher armed.
        ///
        /// It establishes the absolute baseline immediately before the
        /// current save, but must not acknowledge native deltas: the following
        /// current snapshot is the one that reconciles those deltas.
        armed_baseline: bool,
        /// Whether this source authoritatively stated season/Blood Pact mode.
        /// Local character saves do not carry that fact, so their progress
        /// snapshots must not silently select a non-seasonal currency purse.
        has_mode: bool,
        season: i64,
        hardcore: i64,
        blood_pact: i64,
...
mod tests {
    ...
    fn a_relic_is_what_the_tables_say_it_is() {
        ...
        assert_eq!(
            resolve_rarity(&json!(7), relic, true, NO_IDENTITY),
            "Angelic"
        );
    }
    ...
    fn the_tables_outrank_the_packet_for_a_named_item() {
        ...
        assert_eq!(
            resolve_rarity(&json!(7), ring, false, NO_IDENTITY),
            "Satanic",
            "a packet claiming Angelic"
        );
        assert_eq!(
            resolve_rarity(&json!(2), ring, false, NO_IDENTITY),
            "Satanic",
            "and one claiming Superior"
        );
        assert_eq!(
            resolve_rarity(&Value::Null, ring, false, NO_IDENTITY),
            "Satanic",
            "and one claiming nothing"
        );
        // an item the tables have never heard of still keeps what it was sent
        assert_eq!(
            resolve_rarity(&json!(7), "No Such Item", false, NO_IDENTITY),
            "Angelic"
        );
    }
    ...
    fn a_name_two_items_answer_to_is_never_angelic_by_default() {
        ...
        assert_eq!(
            crate::items::rarity_by_name("Shrunken Head"),
            Some("Satanic")
        );
        assert_eq!(crate::items::rarity_by_name("Death's Scythe"), Some("Set"));
        for claim in [json!(7), json!(2), Value::Null] {
            assert_eq!(
                resolve_rarity(&claim, "Shrunken Head", false, NO_IDENTITY),
                "Satanic"
            );
            assert_eq!(
                resolve_rarity(&claim, "Death's Scythe", false, NO_IDENTITY),
                "Set"
            );
        }
        ...
        assert_eq!(
            crate::items::tier_by_name("Shrunken Head"),
            5,
            "the charm is S"
        );
        ...
        assert_eq!(
            crate::items::rarity_by_name("Angel"),
            None,
            "the table will not pick"
        );
        assert_eq!(
    ...
    fn an_identity_tells_apart_two_items_of_one_name() {
        ...
        assert_eq!(
            known_item("Angel", false, angel_gun).map(|k| k.tier),
            Some(5)
        );
        assert_eq!(
            known_item("Angel", false, angel_orb).map(|k| k.tier),
            Some(6)
        );
        ...
        assert_eq!(
            resolve_rarity(&json!(7), "Shrunken Head", false, relic),
            "Common"
        );
        assert_eq!(
            known_item("Shrunken Head", false, relic).map(|k| k.tier),
            Some(1)
        );
        assert_eq!(
            resolve_rarity(&json!(7), "Shrunken Head", false, (10, 37, 0)),
            "Satanic"
        );
        ...
        assert!(
            known_item("", false, angel_gun).is_none(),
            "a nameless base is not this"
        );
        assert!(
            known_item("Angel", true, angel_gun).is_none(),
            "Odyssey numbers its own"
        );
    }
        ...
        for line in BufReader::new(file).lines() {
            ...
            for event in events_from_messages(std::slice::from_ref(&value)) {
                ...
                *kinds.entry(kind).or_default() += 1;
                ...
                let packet = crate::stats::rarity_from_packet(rarity).unwrap_or_else(|| "-".into());
                *found
                    .entry(name.clone())
                    .or_default()
                    .entry((said, packet))
...
where the tables and the packet disagree:"
        ...
        let mut rows: Vec<_> = found
            ...
            .collect();
        rows.sort_by_key(|(name, (said, _), n)| {
            (said.clone(), std::cmp::Reverse(**n), (*name).clone())
        });
        for (name, (said, packet), n) in &rows {
            println!(
                "  {n:>4}x  {said:<9} {:<2} {name} (the packet said {packet})",
                grade(name)
            );
        }
...
named things, by what the tracker made of them:"
    ...
    fn a_real_drop_packet_named_by_two_items_is_not_announced_as_angelic() {
        ...
        let (rarity, name, id) =
            found(dropped("7-4964607-65a04f84c51d80001-10", 37, 0)).expect("a named drop");
        ...
        let (rarity, name, id) =
            found(dropped("7-4964607-65a04f84c51d80001-3", 11, 14)).expect("a named drop");
    ...
    fn an_odyssey_pickup_claims_no_rarity() {
        ...
        assert_eq!(
            resolve_rarity(rarity, name, *unscaled, NO_IDENTITY),
            "Unknown",
